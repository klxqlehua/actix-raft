...
impl<D: AppData, R: AppDataResponse, E: AppError> ClientRequestEntry<D, R, E> {
    ...
    pub(crate) fn from_entry(entry: Entry<D>, response_mode: ResponseMode, tx: TxClientResponse<R, E>) -> Self {
...
impl<'a, D: AppData, R: AppDataResponse, E: AppError, N: RaftNetwork<D, E>, S: RaftStorage<D, R, E>> LeaderState<'a, D, R, E, N, S> {
    ...
    pub(super) async fn commit_initial_leader_entry(&mut self) -> Result<(), E> {
        // If the cluster has just formed, and the current index is 0, then commit the current
        // config, else a blank payload.
        let req: ClientRequest<D, R, E> = if self.core.last_log_index == 0 {
    ...
    pub(super) async fn handle_client_request(&mut self, rpc: ClientRequest<D, R, E>, tx: TxClientResponse<R, E>) {
    ...
    /// Transform the given payload into an entry with assigned index & term, and append the entry to the log.
    pub(super) async fn append_payload_to_log(&mut self, payload: EntryPayload<D>) -> Result<Entry<D>, E> {
    ...
    pub(super) async fn replicate_client_request(&mut self, req: ClientRequestEntry<D, R, E>) {
        ...
        if self.nodes.len() > 0 {
            ...
            for _node in self.nodes.values() {
                // node.replicate(entry_arc.clone()); // TODO: impl this.
                todo!()
            }
            return;
        }
        ...
        match &req.response_mode {
            ...
            ResponseMode::Applied => {
                match self.core.apply_entry_to_state_machine(&req.entry).await {
                    ...
                    Err(err) => {
                        let _ = req.tx.send(Err(RaftError::from(err)));
                    }
                }
            }
        }
